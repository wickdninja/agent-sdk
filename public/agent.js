...
class RealtimeVoiceAgent {
    constructor() {
        ...
        this.endSessionButton = document.getElementById('end-session-button');
    ...
    initializeEventListeners() {
        ...
        if (this.endSessionButton) {
            this.endSessionButton.addEventListener('click', () => this.disconnect());
        }
    }
...
Remember: Keep the conversation natural and friendly. You're having a real conversation, not reading from a script.`,
            ...
            switch (functionName) {
                ...
                case 'submit_order':
                    if (!this.currentUserId || !this.currentOrder.items.length) {
                        ...
                        setTimeout(() => {
                            // Disconnect and return to initial state for next customer
                            this.disconnect();
    ...
    startOver() {
        // Clear conversation but keep session active
        this.addMessage("System", "Starting over with a fresh conversation...", true);
        
        // Reset user and order data
        this.currentUserId = null;
        this.currentOrder = { items: [] };
        this.currentContext = {};
        ...
        if (this.dc && this.dc.readyState === 'open') {
            // Send session update to reset context
            this.sendSessionUpdate();
            
            // Clear chat messages
            setTimeout(() => {
                this.clearChat();
                this.addMessage('Bella', 'Hi there! Starting fresh. Welcome to Brew & Byte Café! I\'m Bella, your barista. What\'s your name?', false);
