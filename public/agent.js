...
Remember: Keep the conversation natural and friendly. You're having a real conversation, not reading from a script.`,
    ...
    handleRealtimeEvent(event) {
        ...
        // Clear response timeout if we're getting responses
    ...
    startConnectionMonitor() {
        ...
        this.connectionMonitorInterval = setInterval(() => {
            if (this.isConnected) {
                ...
                if (timeSinceLastResponse > 30000) { // 30 seconds without response
                    console.warn('[Monitor] No response for 30 seconds, checking connection...');
                    this.checkConnectionHealth();
                }
                
                // Check WebRTC connection state
                if (this.pc && (this.pc.connectionState === 'failed' || this.pc.connectionState === 'disconnected')) {
                    console.warn('[Monitor] Connection state is not connected:', this.pc.connectionState);
                    this.handleConnectionFailure();
                }
            }
        }, 5000);
    ...
    setResponseTimeout() {
        ...
        // Set a 15-second timeout for getting a response
        this.responseTimeout = setTimeout(() => {
            console.warn('[Timeout] No response received within 15 seconds');
            this.addMessage('System', 'Response timeout. Checking connection...', true);
            ...
            if (this.dc && this.dc.readyState === 'open') {
                ...
                // If still no response after another 10 seconds, check connection
                setTimeout(() => {
                    if (this.responseTimeout) {
                        this.checkConnectionHealth();
                    }
    ...
    checkConnectionHealth() {
        // Send a ping-like message to check if connection is alive
        if (this.dc && this.dc.readyState === 'open') {
            try {
                // Try to create a response to see if the connection is still active
                this.sendDataChannelMessage({
                    type: 'response.create',
                    response: {
                        modalities: ['text', 'audio']
                ...
                console.error('[Health Check] Failed to send message:', error);
                this.handleConnectionFailure();
            }
        } else {
            console.warn('[Health Check] Data channel not open');
            this.handleConnectionFailure();
        }
    }
    ...
    handleConnectionFailure() {
        ...
        if (!this.isReconnecting && this.reconnectAttempts < this.maxReconnectAttempts) {
            ...
            this.addMessage('System', 'Connection lost. Please refresh the page or click "Start Over" to reconnect.', true);
            this.updateConnectionStatus('disconnected');
        }
    }
    ...
    async attemptReconnect() {
        ...
        this.addMessage('System', `Connection lost. Reconnecting... (Attempt ${this.reconnectAttempts}/${this.maxReconnectAttempts})`, true);
        this.updateConnectionStatus('connecting');
        ...
        try {
            ...
            await this.connect();
            this.isReconnecting = false;
            this.addMessage('System', 'Reconnected successfully!', true);
        } catch (error) {
            ...
            if (this.reconnectAttempts < this.maxReconnectAttempts) {
                ...
                this.addMessage('System', 'Unable to reconnect. Please refresh the page.', true);
            }
        }
    }
    ...
    disconnect() {
        ...
        this.isConnected = false;
        this.sessionActive = false;
        this.isReconnecting = false; // Prevent auto-reconnect on manual disconnect
